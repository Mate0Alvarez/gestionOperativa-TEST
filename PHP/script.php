<?php
// Establecemos un limite de tiempo (0) para que el script no falle en caso de tomarse demasiado tiempo
set_time_limit(0);
...
foreach( $PRODUCTS AS $PRODUCT){
	...
	$NAME = $RESULTS[0]["name"];
	// INSERTAMOS TODOS LOS TADOS EN EL ARRAY
	$PRODUCTS_LOG[]= [
...
for ($i=0; $i < ($PAGING/50); $i++) {
	...
	foreach( $PRODUCTS AS $PRODUCT){
		...
		// DECLARAMOS LA VARIABLE NAME
		$NAME = $RESULTS[0]["name"];
		// INSERTAMOS TODOS LOS TADOS EN EL ARRAY $PRODUCTS_LOG
		$PRODUCTS_LOG[]= [
			"Id del ítem" => $PRODUCT["id"],
			"Título del ítem" => $PRODUCT["title"],
			"Categoría_ID donde está publicado" => $PRODUCT["category_id"],
